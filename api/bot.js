...
// --- SCENES SETUP FOR CONVERSATIONS (THE ROBUST FIX IS HERE) ---
...
const addCreditStep2 = async (ctx) => {
    ...
    // THE CRITICAL FIX: Save the validated ID into the wizard's memory (state)
    ...
    return ctx.wizard.next(); // Move to the final function
};
...
const addCreditStep3 = async (ctx) => {
    ...
    // THE CRITICAL FIX: Retrieve the saved ID from the wizard's memory
    ...
    return ctx.scene.leave(); // End the conversation successfully
};

// Create the Add Credit Wizard Scene from the steps
...
// Create the Broadcast Scene (single-step conversation)
...
// Create a Stage, which is a scene manager for all conversational commands
